...
const rabbitContent = (intro: string, practice: string, takeaway: string): TopicContent => ({ intro, practice, takeaway, nodes: ["Producer", "RabbitMQ", "Consumer"] });

Object.assign(topicCopy, Object.fromEntries(([
  ["Why asynchronous messaging", "Messaging lets producers hand off work without waiting for consumers, smoothing bursts and isolating failures.", "Define the business event or command, acceptable delay, ownership, and failure policy before introducing a broker.", "Use a queue for a real temporal or ownership boundary, not merely to move a function call."],
  ["RabbitMQ architecture", "RabbitMQ nodes host exchanges, queues, bindings, connections, and Erlang processes; clusters share metadata while queue replicas hold message state.", "Draw publishers, routing topology, queue leaders and replicas, consumers, and every network boundary.", "Availability comes from replicated queue types and sound client behavior, not clustering alone."],
  ["AMQP 0-9-1 model", "AMQP 0-9-1 separates message publication, exchange routing, queue storage, and consumer delivery.", "Model each publish as exchange plus routing key and each delivery as a queue subscription.", "Publishers address exchanges; consumers read queues."],
  ["Connections & channels", "A TCP connection is expensive and long-lived; lightweight channels multiplex protocol operations but are not thread-safe.", "Reuse a bounded connection pool and give concurrent publishing or consuming flows their own channels.", "Keep connections long-lived and channel ownership explicit."],
  ["Virtual hosts", "A virtual host isolates topology, permissions, and policies inside one broker or cluster.", "Separate environments or trust domains with vhosts and grant users only required configure, write, and read patterns.", "Treat a vhost as a security boundary, not a substitute for capacity isolation."],
  ["Declaring topology", "Declarations create exchanges, queues, and bindings and can safely be repeated only when attributes match.", "Declare topology at deployment or startup, fail fast on inequivalent arguments, and version breaking changes.", "Topology is infrastructure and belongs in reviewable automation."],
  ["Direct exchanges", "A direct exchange routes when a binding key exactly matches the message routing key.", "Use stable keys such as invoice.created when exact routing is required.", "Choose direct routing when equality expresses the contract."],
  ["Fanout exchanges", "A fanout exchange copies every publication to every bound queue and ignores routing keys.", "Give each subscriber its own durable queue so consumers can progress independently.", "Fanout broadcasts to queues, not directly to consumer processes."],
  ["Topic exchanges", "Topic exchanges match dot-delimited routing keys with * for one segment and # for zero or more.", "Design a small documented vocabulary and test bindings against representative keys.", "Keep routing keys stable, bounded, and free of sensitive data."],
  ["Headers exchanges", "Headers exchanges route using message header predicates rather than routing-key patterns.", "Use them only when multi-attribute matching clearly outweighs added opacity and overhead.", "Prefer simpler direct or topic routing unless headers are essential."],
  ["Default exchange & bindings", "The nameless default exchange routes directly to a queue whose name equals the routing key.", "Use it for deliberate point-to-point delivery, while remembering every queue receives an automatic binding.", "An empty exchange name is special broker behavior, not a missing configuration."],
  ["Message properties & metadata", "Properties carry content type, message ID, correlation ID, timestamp, expiration, and delivery mode alongside the payload.", "Standardize an envelope with identity, causation, schema version, trace context, and content type.", "Metadata must support diagnosis and compatibility without leaking secrets."],
  ["Publisher confirms", "Confirms asynchronously report whether the broker accepted a publish on a channel.", "Track sequence numbers, batch or pipeline publishes, handle nack and channel failure, and bound outstanding confirms.", "A successful API call is not proof of broker acceptance; wait for confirms."],
  ["Mandatory publishing & returns", "The mandatory flag returns a message that no queue binding can route.", "Register the return handler before publishing and correlate returns with the original operation.", "Detect unroutable messages instead of silently dropping them."],
  ["Durable topology & persistent messages", "Durable queues and exchanges survive restart; persistent delivery mode asks a durable queue to persist message state.", "Combine durable replicated queues, persistent messages, and confirms, then test node loss.", "No single durability flag provides end-to-end safety."],
  ["Message ordering", "A queue is ordered, but redelivery, priorities, multiple consumers, and asynchronous work can change observed completion order.", "Partition by entity, use single-active-consumer where strict order matters, or make operations commutative.", "State the scope of ordering rather than promising global order."],
  ["Publisher connection recovery", "Recovery recreates connections and topology, but in-flight outcomes may remain ambiguous.", "Reconnect with jitter, redeclare topology, resume confirms, and republish uncertain work from an outbox idempotently.", "Reconnect automation cannot decide whether an ambiguous publish committed."],
  ["Consumer acknowledgements", "Manual ack removes a delivery only after the application confirms successful processing.", "Acknowledge after durable side effects; nack or close the channel when the outcome is unsafe.", "Ack timing defines the loss-versus-duplication tradeoff."],
  ["Prefetch & flow control", "Prefetch limits unacknowledged deliveries and controls memory, fairness, and parallelism.", "Start near actual worker concurrency, observe latency and utilization, then tune with payload size in mind.", "Unlimited in-flight work turns a consumer into an accidental buffer."],
  ["Competing consumers", "Consumers on one queue share deliveries to scale work horizontally.", "Keep handlers stateless or partition state, set prefetch deliberately, and expect uneven work durations.", "Scale consumers only until the queue, downstream dependency, or ordering constraint becomes limiting."],
  ["Idempotent consumers", "At-least-once delivery means a completed message can appear again after an uncertain acknowledgement.", "Persist a message ID or business idempotency key atomically with the side effect.", "Design every externally visible handler to tolerate duplicates."],
  ["Consumer cancellation", "The broker can cancel a consumer when its queue disappears, leadership changes, or policy requires it.", "Handle cancellation notifications, stop accepting work, surface health, and resubscribe only when safe.", "Cancellation is a state transition, not merely an exception to log."],
  ["Graceful shutdown", "Shutdown must stop new deliveries while allowing bounded in-flight work to finish or requeue.", "Cancel consumption, await handlers until a deadline, ack completed work, close channels, then connections.", "Drain in dependency order and make forced termination safe."],
  ["Negative acknowledgements", "Nack and reject can discard or requeue failed deliveries; immediate requeue can create a hot loop.", "Classify transient and permanent failures and route retries through bounded delay queues.", "Never blindly requeue an unchanged poison message."],
  ["Dead-letter exchanges", "Queues dead-letter rejected, expired, over-limit, or delivery-limited messages to another exchange.", "Configure policies, preserve failure context, and bind a monitored quarantine queue.", "Dead lettering moves a message; it does not resolve the failure."],
  ["Retry topologies", "Retries need delay, attempt limits, and a terminal destination.", "Use TTL delay queues or a delayed-message capability, increment attempt metadata, and add jitter where possible.", "Bound retries by count and elapsed time."],
  ["Poison messages", "A poison message deterministically fails because its payload, schema, or business state cannot be handled.", "Quarantine it with redacted diagnostics and provide controlled inspect, repair, replay, and discard operations.", "Keep poison traffic from consuming normal capacity."],
  ["Time-to-live & expiration", "Queue and per-message TTL expire stale work; expired messages may be dead-lettered.", "Base TTL on business value, account for head-of-line behavior, and monitor expiration rates.", "Expiration is a product decision as well as a broker setting."],
  ["Quorum queue delivery limits", "Quorum queues can cap repeated redeliveries and dead-letter messages that exceed the limit.", "Set a policy aligned with retry design and alert before discard or quarantine grows unnoticed.", "Broker limits are the final guardrail, not the retry strategy."],
  ["Work queues", "A work queue distributes independent commands among competing workers.", "Use durable commands, manual acknowledgements, idempotent effects, bounded prefetch, and measurable backlog age.", "One command should have one clear owner and outcome."],
  ["Publish-subscribe", "Publish-subscribe gives each interested capability an independent copy through its own queue.", "Publish immutable facts and let subscribers own retries, retention, and deployment cadence.", "Events describe what happened; they do not command every subscriber."],
  ["Request-reply", "Request-reply emulates a response using reply-to and correlation metadata but retains distributed failure modes.", "Set deadlines, correlate exactly once, clean up replies, and prefer direct reply-to only for appropriate transient clients.", "Do not hide a latency-sensitive RPC chain behind queues."],
  ["Correlation identifiers", "Correlation, message, and causation IDs connect a reply or event to the operation that produced it.", "Generate immutable IDs at boundaries and propagate them through logs, traces, retries, and outgoing messages.", "Identifiers make workflows explainable; they must not carry authority."],
  ["Saga messaging", "A saga coordinates local transactions through messages and compensations instead of a distributed database transaction.", "Model states, commands, events, timeouts, idempotency, and compensations explicitly.", "A compensation is a business action, not a rollback."],
  ["Outbox & inbox patterns", "An outbox commits outgoing intent with domain data; an inbox deduplicates incoming work with its effect.", "Relay committed outbox rows with confirms and mark them sent idempotently; record inbox IDs transactionally.", "Bridge database and broker guarantees with durable application state."],
  ["Classic vs quorum queues", "Classic queues favor simpler storage; quorum queues use Raft replication for data safety and predictable failover.", "Default critical durable workloads to quorum queues and validate feature and performance needs.", "Choose queue type from failure guarantees, not habit."],
  ["Streams & super streams", "Streams retain an append-only log for replay; super streams partition it for parallel scale.", "Select retention, partition key, offset tracking, and consumer protocol from replay requirements.", "Use streams when retained history and replay are first-class needs."],
  ["Lazy queue behavior", "Modern RabbitMQ queues page messages based on memory pressure; older lazy-mode advice is version-dependent.", "Capacity-test disk-heavy backlogs on the deployed RabbitMQ version and storage hardware.", "Tune current behavior from measurements, not legacy flags."],
  ["Single active consumer", "Single active consumer keeps one consumer active and promotes another after failure.", "Use it when per-queue processing order matters and handlers can tolerate redelivery during promotion.", "It serializes consumption for one queue and therefore limits throughput."],
  ["Priority queues", "Priority queues deliver higher-priority messages first but add internal queues and can starve normal work.", "Use few priority levels, combine with prefetch, and reserve capacity for ordinary traffic.", "Separate queues are often clearer when service classes differ materially."],
  ["Backpressure & capacity planning", "Backpressure prevents ingress from exceeding broker, consumer, or dependency capacity indefinitely.", "Measure publish rate, processing rate, backlog age, bytes, disk headroom, and recovery time after a burst.", "Size for drain time and failure recovery, not average throughput."],
  ["TLS & client authentication", "TLS protects credentials and payloads in transit and can authenticate clients with certificates.", "Verify hostnames, rotate certificates, restrict protocols and ciphers, and test expiry alarms.", "Encryption without identity verification still permits interception."],
  ["Users, permissions & vhosts", "RabbitMQ permissions separately govern configure, write, and read operations using resource patterns.", "Create per-application identities, deny default broad access, and audit effective permissions.", "Least privilege limits both accidents and compromised clients."],
  ["Resource alarms", "Memory and disk alarms block publishers to protect the node from exhaustion.", "Set realistic watermarks, monitor headroom, and make publishers tolerate blocked connections without unbounded local buffering.", "A broker protecting itself must not cascade failure into clients."],
  ["Metrics & observability", "Useful signals cover rates, backlog age, ready and unacked counts, confirms, returns, redeliveries, resources, and node health.", "Create symptom-based alerts and propagate trace context without putting secrets in headers.", "Observe message outcomes and business delay, not only node uptime."],
  ["Clustering & partitions", "A cluster shares topology while queue replicas coordinate data; network partitions force availability and consistency choices.", "Place nodes across failure domains with low latency, use quorum queues, and rehearse partitions and minority loss.", "A stretched cluster is not a substitute for federation between distant regions."],
  ["Backup, recovery & upgrades", "Definitions, users, policies, messages, and application state have different backup and recovery mechanisms.", "Export definitions securely, protect credentials, rely on replication appropriately, and rehearse rolling upgrades and full rebuilds.", "A backup is credible only after a timed restore test."],
  ["High availability design", "High availability combines replicated queues, odd-sized quorum groups, failure-domain placement, and reconnecting clients.", "Define RPO and RTO, remove single dependencies, and chaos-test node, zone, network, and client failures.", "Design the whole message path for failure, not only the broker cluster."],
  ["Federation & Shovel", "Federation links exchanges or queues loosely across brokers; Shovel moves messages through an explicit client-like transfer.", "Use federation for ongoing topology relationships and Shovel for controlled movement or migration.", "Prefer loose inter-cluster links over latency-sensitive stretched clusters."],
  ["Schema evolution", "Queued messages outlive deployments, so producers and consumers must overlap across schema versions.", "Use additive changes, explicit versions, tolerant readers, fixtures, and staged removal of old fields.", "Compatibility must span the maximum message lifetime and replay window."],
  ["Testing message-driven systems", "Tests must cover handlers, topology, serialization, redelivery, duplicates, ordering assumptions, and real broker behavior.", "Layer pure unit tests with containerized integration tests and focused failure scenarios.", "Mock clients prove code paths; a real broker proves protocol assumptions."],
  ["Performance tuning", "Throughput and latency depend on payloads, confirms, batching, prefetch, queue type, persistence, disk, and consumers.", "Benchmark representative traffic, vary one control at a time, and include failure recovery and tail latency.", "Optimize the bottleneck you measured while preserving required guarantees."],
  ["Production readiness review", "A readiness review verifies ownership, guarantees, limits, security, telemetry, runbooks, recovery, and compatibility.", "Document topology, SLOs, dashboards, alert actions, replay controls, capacity thresholds, and incident contacts.", "A messaging system is ready when the team can explain and recover every ambiguous outcome."]
] as [string, string, string, string][]).map(([topic, intro, practice, takeaway]) => [topic, rabbitContent(intro, practice, takeaway)])));
...
// Course 12: Test-Driven Development
Object.assign(topicCopy, {
  "What is Test-Driven Development?": authContent("Test-Driven Development (TDD) is a development approach in which a test is written before the production code. The test defines one observable behavior, and the implementation grows only enough to satisfy it.", "Instead of completing a feature and testing it afterward, move in tiny examples: describe the next behavior with a test, watch it fail for the expected reason, implement it, and improve the design while the suite stays green.", "Use tests to guide behavior and design, not merely to verify finished code."),
  "Test-first development": authContent("Test first means production code is written in response to an existing failing test. This forces the expected input, output, and edge case to become concrete before implementation decisions take over.", "Name the behavior in domain language, arrange only the necessary context, perform one action, and assert the meaningful outcome. Confirm the test fails because the capability is missing—not because the test is broken.", "Every production change should answer a specific failing behavioral example."),
  "Red: write a failing test": authContent("Red begins with the smallest test that expresses a missing requirement. Running it proves that the suite can detect the absent behavior and establishes a clear next target.", "For a price calculator, first assert that two $25 items total $50. A useful red result says the calculator or behavior is missing; a syntax error or faulty setup is not meaningful Red.", "Watch the test fail for the reason you expect before writing the implementation."),
  "Green: make the test pass": authContent("Green means writing the minimum production code needed to pass the current test. Elegance, generalization, and speculative features wait until the suite confirms the requested behavior.", "Implement the simplest sum that satisfies the example, run the focused test, then run the complete fast suite. A deliberately direct solution keeps the feedback loop short and prevents unrequested design.", "Make the current behavior work with the smallest honest change."),
  "Refactor: improve the design": authContent("Refactoring changes structure without changing observable behavior. With all tests green, remove duplication, improve names, clarify responsibilities, and simplify both implementation and test code.", "Extract a Money type only when repeated currency rules make it useful; rename vague helpers; consolidate noisy setup. Run the suite after each small transformation so a failure identifies the latest change.", "Improve structure only under a green test suite, in small reversible steps."),
  "The rapid feedback loop": authContent("Red–Green–Refactor is intentionally short. Frequent execution exposes regressions near their cause and turns development into a sequence of small, evidence-backed decisions.", "Keep the focused test available in seconds, run nearby tests as the change grows, and run the broader suite before integration. If one cycle takes hours, reduce the behavior or remove slow infrastructure from the unit boundary.", "Optimize the loop for fast, trustworthy feedback rather than maximum test volume."),
  "Minimal implementation": authContent("Minimal implementation means adding only what the current example requires. It resists speculative abstractions and lets later tests reveal the general design through concrete pressure.", "If the only requirement is summing valid item prices, do not add discounts, taxes, currencies, persistence, or plugin interfaces. Add each capability when a failing test makes its behavior explicit.", "Let successive examples earn complexity; do not design for imagined requirements."),
  "Granular behavior": authContent("TDD works best when each step covers one small observable behavior. Granularity makes failures diagnostic and keeps implementation changes easy to understand and reverse.", "Separate examples for an empty cart, multiple items, and an invalid negative price. Avoid one test that performs an entire checkout and asserts unrelated totals, inventory, payment, and email effects.", "One test may contain several assertions when they describe one behavior, but avoid mixing independent reasons to fail."),
  "Continuous feedback": authContent("Continuous feedback comes from running tests throughout development rather than at the end. A fast suite catches regressions quickly and preserves confidence during refactoring and dependency upgrades.", "Run the current test on save, the relevant unit suite after each cycle, and broader integration checks before merging. Keep nondeterministic time, networks, and global state outside fast unit tests.", "A test suite is useful when developers trust it and run it frequently."),
  "Discount calculator scenario": authContent("The exercise is to calculate a shopping-cart subtotal after applying a percentage discount. The first requirement is deliberately small: applying a 10% discount to $100 must return $90.", "Use the same behavior to compare TDD in C# with xUnit and JavaScript with Jest. In both languages, begin with the expectation, add only enough calculation to pass, then clarify validation and rounding while keeping the test green.", "Keep the requirement constant so the Red–Green–Refactor mechanics—not language differences—remain the focus."),
  "Red–Green–Refactor with C# and xUnit": authContent("In Red, an xUnit fact constructs DiscountCalculator and expects CalculateSubtotal(100.00m, 10.0) to equal 90.00m; compilation fails because the class does not exist. In Green, the smallest percentage calculation makes it pass.", "During Refactor, validate that the percentage is between 0 and 100, express the multiplier with decimal arithmetic, and round the monetary result to two decimal places. Re-run the original test after every structural improvement.", "A compile failure is valid Red when it directly identifies the missing type or member required by the test."),
  "Red–Green–Refactor with JavaScript and Jest": authContent("In Red, a Jest test imports calculateSubtotal and expects calculateSubtotal(100, 10) to be 90; it fails because the module or function is missing. In Green, export the smallest percentage calculation that satisfies the example.", "During Refactor, reject percentages outside 0–100, name the factor and total clearly, and use toFixed(2) followed by Number to control the result’s two-decimal rounding in the number-based example. Run npm test after each step.", "Keep the first test green while added edge-case tests justify validation and precision behavior."),
  "Choosing valuable tests": authContent("Valuable tests describe externally meaningful behavior at a stable boundary. They fail when the requirement breaks, not whenever an internal helper, call order, or data structure changes.", "Assert the returned total or domain error rather than spying on Array.reduce. Prefer representative boundaries and business rules over exhaustive combinations that add little confidence.", "Test what the code promises, not how the current implementation happens to deliver it."),
  "Keeping tests resilient": authContent("Resilient tests use clear names, focused setup, stable public interfaces, and deterministic inputs. Fragile tests mirror implementation details and turn harmless refactoring into widespread rewrites.", "Use builders for meaningful defaults, inject clocks and gateways at real boundaries, and avoid broad mocks. Keep assertion messages and fixtures readable enough that a failure explains the broken behavior.", "Treat test code as production-quality documentation: readable, intentional, and maintainable."),
  "TDD advantages": authContent("TDD clarifies requirements before implementation, supplies a safety net for refactoring, creates executable examples, and encourages modular designs with explicit dependencies and low coupling.", "Use the suite to change a pricing rule confidently, show a new teammate supported behavior, and identify design friction when a unit is difficult to exercise without excessive setup.", "The main benefit is continuous design feedback and confidence to change—not a test-count target."),
  "TDD challenges": authContent("TDD can initially feel slower because test infrastructure and examples are developed alongside the feature. Poorly scoped tests can also create a maintenance burden by coupling to internals or duplicating implementation logic.", "Invest in fast tooling, learn to slice behaviors smaller, delete redundant tests, and refactor test code. Do not force every trivial declaration or exploratory spike through a ceremonial cycle.", "Apply TDD deliberately and keep the suite economical, behavioral, and fast."),
  "TDD and the wider test strategy": authContent("TDD primarily shapes units and behavioral specifications; it is not a complete quality-assurance strategy. Unit confidence cannot prove database mappings, network contracts, performance, accessibility, or that users can complete a real workflow.", "Combine many fast unit tests with targeted integration and contract tests, a small set of end-to-end journeys, and appropriate performance, security, accessibility, and user-acceptance checks.", "Use TDD as one feedback layer within a risk-based testing strategy."),
});

Object.assign(codeExamples, {
  "What is Test-Driven Development?": authExample("Let behavior pull implementation into existence", "Code first, tests later", `export function calculateTotal(items: Item[]) {\n  // Large implementation written from assumptions.\n}\n\n// Tests are added after the design is fixed.`, "Test first", `it("totals the item prices", () => {\n  expect(calculateTotal([\n    { price: 25 }, { price: 25 }\n  ])).toBe(50)\n})\n\n// Run it, see Red, then implement.`),
  "Test-first development": authExample("Describe one missing behavior", "Test an internal procedure", `expect(reduceSpy).toHaveBeenCalledTimes(1)`, "Test the public result", `expect(calculateTotal([\n  { price: 12 }, { price: 8 }\n])).toBe(20)`),
  "Red: write a failing test": authExample("Prove the behavior is absent", "A meaningless failure", `it("totals prices", () => {\n  expect(calculteTotal(items)).toBe(50)\n})\n// Fails because the test has a typo.`, "A meaningful failing test", `it("totals the item prices", () => {\n  expect(calculateTotal([\n    { price: 25 }, { price: 25 }\n  ])).toBe(50)\n})\n// FAIL: calculateTotal is not implemented.`),
  "Green: make the test pass": authExample("Implement only the requested behavior", "Speculative feature set", `class PricingEngine {\n  // discounts, taxes, currencies, plugins...\n}`, "Smallest honest implementation", `type Item = { price: number }\n\nexport function calculateTotal(items: Item[]) {\n  return items.reduce((total, item) => total + item.price, 0)\n}`),
  "Refactor: improve the design": authExample("Change structure with behavior fixed", "Mix behavior changes into cleanup", `// Rename, optimize, and add discount behavior together.`, "Refactor under green", `const sumPrices = (items: Item[]) =>\n  items.reduce((total, { price }) => total + price, 0)\n\nexport const calculateTotal = (items: Item[]) => sumPrices(items)\n\n// Existing tests remain green.`),
  "The rapid feedback loop": authExample("Keep each cycle small", "One enormous red phase", `write 30 tests → implement entire feature → debug for hours`, "Continuous micro-cycles", `RED      one missing behavior\n  ↓\nGREEN    smallest passing change\n  ↓\nREFACTOR improve names and structure\n  ↺       repeat in minutes`),
  "Minimal implementation": authExample("Let tests earn complexity", "Build tomorrow's requirements", `calculateTotal(items, discounts, taxes, currencies, region, plugins)`, "Meet today's example", `export const calculateTotal = (items: Item[]) =>\n  items.reduce((sum, item) => sum + item.price, 0)`),
  "Granular behavior": authExample("Give each rule a clear failure", "One test for an entire workflow", `it("checks out", async () => {\n  // total + stock + payment + email + analytics\n})`, "Focused behavioral examples", `it("returns zero for an empty cart", () => { ... })\nit("totals multiple items", () => { ... })\nit("rejects a negative price", () => { ... })`),
  "Continuous feedback": authExample("Layer feedback by speed", "Wait until the end", `implement for two days\nnpm test\n// 47 failures with unclear causes`, "Run continuously", `on save       → focused unit test\nafter cycle   → related unit suite\nbefore merge  → integration + full checks`),
  "Discount calculator scenario": authExample("Define one observable pricing rule", "Broad shopping-cart scope", `discounts + taxes + coupons + shipping + refunds`, "First requirement", `Given:  original price = $100\nAnd:    discount = 10%\nWhen:   the subtotal is calculated\nThen:   the result is $90`),
  "Red–Green–Refactor with C# and xUnit": authExample("Grow the C# calculator through the cycle", "Write the finished class before its test", `public class DiscountCalculator\n{\n    // Validation and implementation designed upfront.\n}`, "xUnit: Red → Green → Refactor", `// RED — DiscountCalculator does not exist yet\n[Fact]\npublic void CalculateSubtotal_AppliesPercentageDiscount_Correctly()\n{\n    var calculator = new DiscountCalculator();\n    decimal result = calculator.CalculateSubtotal(100.00m, 10.0);\n    Assert.Equal(90.00m, result);\n}\n\n// GREEN — minimum passing implementation\npublic decimal CalculateSubtotal(decimal price, double percentage)\n{\n    return price - (price * (decimal)(percentage / 100.0));\n}\n\n// REFACTOR — explicit bounds and monetary rounding\npublic decimal CalculateSubtotal(decimal price, double percentage)\n{\n    if (percentage < 0 || percentage > 100)\n        throw new ArgumentOutOfRangeException(nameof(percentage),\n            "Discount must be between 0 and 100.");\n\n    decimal multiplier = 1m - ((decimal)percentage / 100m);\n    return Math.Round(price * multiplier, 2);\n}`),
  "Red–Green–Refactor with JavaScript and Jest": authExample("Grow the JavaScript calculator through the cycle", "Implement before stating the behavior", `function calculateSubtotal(price, percentage) {\n  // Full implementation written first.\n}`, "Jest: Red → Green → Refactor", `// RED — module/function is missing\ntest("applies percentage discount correctly", () => {\n  expect(calculateSubtotal(100, 10)).toBe(90)\n})\n\n// GREEN — minimum passing implementation\nfunction calculateSubtotal(price, percentage) {\n  return price - (price * (percentage / 100))\n}\n\n// REFACTOR — bounds, names, and rounding\nfunction calculateSubtotal(price, percentage) {\n  if (percentage < 0 || percentage > 100) {\n    throw new RangeError(\n      "Discount percentage must be between 0 and 100."\n    )\n  }\n\n  const factor = 1 - (percentage / 100)\n  const total = price * factor\n  return Number(total.toFixed(2))\n}\n\nmodule.exports = { calculateSubtotal }`),
  "Choosing valuable tests": authExample("Assert the contract", "Mirror implementation details", `expect(reduceSpy).toHaveBeenCalled()\nexpect(validateSpy).toHaveBeenCalledBefore(sumSpy)`, "Describe meaningful behavior", `expect(calculateTotal([{ price: 10 }, { price: 5 }])).toBe(15)\nexpect(() => calculateTotal([{ price: -1 }])).toThrow()`),
  "Keeping tests resilient": authExample("Depend on stable boundaries", "Mock every collaborator", `mock(array.reduce)\nmock(privateValidator)\nmock(privateFormatter)`, "Use deterministic domain inputs", `const item = (price: number) => ({ price })\n\nit("totals items", () => {\n  expect(calculateTotal([item(10), item(5)])).toBe(15)\n})`),
  "TDD advantages": authExample("Turn examples into a safety net", "Refactor by hope", `changePricingRules()\ndeploy()\n// Discover regressions later.`, "Refactor with evidence", `changePricingRules()\nrunUnitSuite()       // behavior preserved\nrunIntegrationSuite() // boundaries preserved`),
  "TDD challenges": authExample("Maintain tests as a product", "Accumulate brittle coverage", `100 tests assert private calls\n// Every refactor rewrites the suite.`, "Keep the suite economical", `• Delete redundant examples\n• Test stable behavior\n• Isolate slow infrastructure\n• Refactor fixtures and builders`),
  "TDD and the wider test strategy": authExample("Cover different risks at different levels", "Treat unit tests as complete QA", `unit tests pass → ship\n// Database contract and user journey untested.`, "Layer the strategy", `many     fast unit tests\nsome     integration + contract tests\nfew      end-to-end journeys\ntargeted performance, security, accessibility, and UAT`),
});
